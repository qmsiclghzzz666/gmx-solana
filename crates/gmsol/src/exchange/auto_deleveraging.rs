...
pub struct AutoDeleverageBuilder<'a, C> {
    ...
    hint: Option<AutoDeleverageHint>,
...
impl<'a, C: Deref<Target = impl Signer> + Clone> AutoDeleverageBuilder<'a, C> {
    ...
    pub async fn prepare_hint(&mut self) -> crate::Result<AutoDeleverageHint> {
        match &self.hint {
            Some(hint) => Ok(hint.clone()),
            None => {
                let position = self.client.position(&self.position).await?;
                let hint = AutoDeleverageHint::from_position(self.client, &position).await?;
    ...
    pub async fn build(&mut self) -> crate::Result<TransactionBuilder<'a, C>> {
        ...
        let exec_builder = self
            .client
            .exchange_rpc()
            .accounts(accounts::AutoDeleverage {
            ...
            .args(instruction::AutoDeleverage {
            ...
            .accounts(feeds)
            .compute_budget(ComputeBudget::default().with_limit(ADL_COMPUTE_BUDGET));

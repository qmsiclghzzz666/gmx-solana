...
import { createRolesPDA, storeProgram } from "../../utils/data";
...
describe("store: Oracle", () => {
    ...
    it("set price from feed and then clear", async () => {
        ...
        await storeProgram.methods.setPricesFromPriceFeed([
            BTC_TOKEN_MINT,
            SOL_TOKEN_MINT,
            fakeTokenMint,
            usdGTokenMint,
        ...
        const setData = await storeProgram.account.oracle.fetch(oracleAddress);
        // console.log(setData.primary.prices);
        expect(setData.primary.prices.length).to.equal(4);

        await storeProgram.methods.clearAllPrices().accountsPartial({
            store: dataStoreAddress,
            authority: signer0.publicKey,
            oracle: oracleAddress,
        }).signers([signer0]).rpc();
        const clearedData = await storeProgram.account.oracle.fetch(oracleAddress);
